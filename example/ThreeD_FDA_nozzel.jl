...
function make_pipe(L=32;stenosis=0.5,U=1,Re=2500,mem=Array,T=Float32)
    ...
    # analytical solution laminar pipe flow u/U ~ (1-r^2/R^2) ∀ y ∈ [0,L/2]
...
function make_FDA_nozzel(L=32;U=1,Re=2500,mem=Array,T=Float32)
    ...
    # pressure gradient required to drive the flow to u~1
    body = AutoBody(pipe)
    Simulation((16L,L,L), u_pipe, L; U=one(T), ν=U*L/Re, body, mem, T, exitBC=true)
end

# sim = make_channel(48;mem=CuArray)
# sim = make_pipe(48;mem=CuArray)
sim = make_FDA_nozzel(48;mem=CuArray)
...
# flood(σ[:,:,25], clims=(-0.3,2), axis=([], false), cfill=:binary,
