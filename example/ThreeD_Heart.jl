...
function heart(L=2^5;Re=5e3,mem=Array,U=1,AR=1,T=Float32)
    ...
    disk = AutoBody((x,t)->√(x[3]^2+(r(x)-min(r(x),L/AR))^2)-1.5f0,
                    (x,t)->x.-SA[-w(t),-w(t),2.25f0*L])
    plane = AutoBody((x,t)->x[3]-2.25f0*L)
    mitral = AutoBody((x,t)->√(x[3]^2+(r(x)-min(r(x),1))^2)-L/4.f0,
                      (x,t)->x.-SA[0,0,2.25f0*L])
    ...
    Simulation((Int(1.25*L),Int(1.5*L),3L),(0f0,0f0,0f0),L;U,ν=U*L/Re,body,mem,T=T),mask
end
...
custom_attrib = Dict("u"=>vtk_velocity,"p"=>vtk_pressure,
                     "λ₂"=>vtk_lamda,"Q"=>vtk_Q,"d"=>vtk_body)# this
