...
TODO:
  ...
  - [x] interpolate mesh in time?
"""

using WaterLily, StaticArrays, WaterLilyMeshBodies
using GeometryBasics: Point3f, TriangleFace, GLTriangleFace
...
function make_sim(;L=64,Re=5000,U=1,mem=Array,T=Float32)
    ...
    motion = [m.+SA{T}[L/2.f0,L/2.f0,5L÷4-maximum(m[3,:])] for m in motion]
    ...
    # make a simulation
    Simulation((L,L,2L),(0,0,0),L;U,ν=U*L/Re,body,mem,T), motion_update, shell
end
...
using CUDA
# make simulation
sim,motion,mesh_body = make_sim(L=256;mem=CuArray)
